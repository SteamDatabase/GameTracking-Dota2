ogreseal_flop = class({})
LinkLuaModifier( "modifier_ogreseal_flop", "heroes/modifiers/modifier_ogreseal_flop", LUA_MODIFIER_MOTION_BOTH )
LinkLuaModifier( "modifier_ogreseal_flying", "heroes/ogreseal_flop", LUA_MODIFIER_MOTION_BOTH )
...
function ogreseal_flop:OnSpellStart()
	if IsServer() then
		...
		self:GetCaster():AddNewModifier(self:GetCaster(), self, "modifier_ogreseal_flying", nil)
	end
end
...
function ogreseal_flop:TryToDamage()
	if IsServer() then
			...
			for _,enemy in pairs(enemies) do
				if enemy ~= nil and ( not enemy:IsMagicImmune() ) and ( not enemy:IsInvulnerable() ) then
					...
					if enemy:IsAlive() == false then
						...
						ParticleManager:SetParticleControlForward( nFXIndex, 1, forwardVector )
		...
		EmitSoundOn("OgreTank.GroundSmash", caster)
		local nFXIndex = ParticleManager:CreateParticle( "particles/test_particle/ogre_melee_smash.vpcf", PATTACH_WORLDORIGIN, caster )
		ParticleManager:SetParticleControl( nFXIndex, 0, casterPosition )
		ParticleManager:SetParticleControl( nFXIndex, 1, Vector( radius, radius, radius ) )
		ParticleManager:ReleaseParticleIndex( nFXIndex )

		GridNav:DestroyTreesAroundPoint( casterPosition, radius, false )
	end
end
...
modifier_ogreseal_flying = class({})

function modifier_ogreseal_flying:IsHidden()
	return true
end

function modifier_ogreseal_flying:CheckState()
	local state =
	{
