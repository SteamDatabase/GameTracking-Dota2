if modifier_ogreseal_flop == nil then
...
function modifier_ogreseal_flop:OnCreated( kv )
	if IsServer() then
		if self.nHopCount == nil then
			self.nMaxHops = self:GetAbility():GetSpecialValueFor("flop_count") + 1
			self.flop_distance = self:GetAbility():GetSpecialValueFor("flop_distance")
			self.nHopCount = 2
		end
		...
		local vToTarget = self:GetCaster():GetForwardVector() * self.flop_distance
		self.vLastKnownTargetPos = self:GetCaster():GetOrigin() + vToTarget

		local flLowZ = math.min( self.vLastKnownTargetPos.z, self.vStartPosition.z )
		local flHighZ = math.max( self.vLastKnownTargetPos.z, self.vStartPosition.z )
		local flArcTopZ = flHighZ + OGRE_MINIMUM_HEIGHT_ABOVE_HIGHEST * self.nHopCount
...
function modifier_ogreseal_flop:OnDestroy()
	if IsServer() then
		...
		if self:GetCaster():GetOwnerEntity():HasModifier("modifier_mounted") then
			-- Owner is still mounted, continue moving
			local hPassiveAbility = self:GetCaster():FindAbilityByName("ogreseal_passive")
			if hPassiveAbility ~= nil then
				self:GetCaster():AddNewModifier(self:GetCaster(), hPassiveAbility, "modifier_mount_movement", {})
			end

			-- Set owner cooldown to the same as ours for a visual indicator of when the next flop is
