...
function modifier_item_candy:CandyScoreCheck()
	...
	if hParent ~= nil and hParent:IsAlive() and hParent:IsNull() == false then
		local hCandyBuckets = GameRules.Winter2022:GetCandyBuckets( hParent:GetTeamNumber() )
...
function modifier_item_candy:DropCandy()
	...
	if nNumBigBags > 0 then
		...
		for i = 1, nNumBigBags do
			GameRules.Winter2022:DropCandyAtPosition( self:GetParent():GetAbsOrigin(), self:GetParent(), nil, true, 1.0 )
		end
	end
	for i = 1, nNumCandy do
		GameRules.Winter2022:DropCandyAtPosition( self:GetParent():GetAbsOrigin(), self:GetParent(), nil, false, 1.0 )
	end
...
function modifier_item_candy:ScoreCandy()
	...
	local nNumCandy = hCandy:GetCurrentCharges()
	GameRules.Winter2022:ScoreCandy( self:GetParent():GetTeamNumber(), self:GetParent(), nNumCandy )
	self:GetParent():RemoveItem( hCandy )
end
