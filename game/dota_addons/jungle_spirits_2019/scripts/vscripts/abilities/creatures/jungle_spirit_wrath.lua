...
jungle_spirit_wrath = class({})
LinkLuaModifier( "modifier_jungle_spirit_wrath_thinker", "modifiers/creatures/modifier_jungle_spirit_wrath_thinker", LUA_MODIFIER_MOTION_NONE )
...
function jungle_spirit_wrath:GetChannelAnimation()
	return ACT_DOTA_CHANNEL_ABILITY_4
end

--------------------------------------------------------------------------------

function jungle_spirit_wrath:OnAbilityPhaseStart()
	if IsServer() then
		...
		self:GetCaster():AddNewModifier( self:GetCaster(), self, "modifier_jungle_spirit_immunity", { duration = fImmuneDuration } )
...
function jungle_spirit_wrath:OnAbilityPhaseInterrupted()
	if IsServer() then
...
function jungle_spirit_wrath:OnSpellStart()
	if IsServer() then	
		ParticleManager:DestroyParticle( self.nPreviewFX, false )
...
function jungle_spirit_wrath:OnChannelThink( flInterval )
	if IsServer() then
		...
		if self.flNextCast >= self.interval  then
			...
			repeat
				...
				for _, hThinker in pairs( hThinkersNearby ) do
					if ( hThinker:HasModifier( "modifier_jungle_spirit_wrath_thinker" ) ) then
			...
			CreateModifierThinker( self:GetCaster(), self, "modifier_jungle_spirit_wrath_thinker", {}, vPos, self:GetCaster():GetTeamNumber(), false )
...
function jungle_spirit_wrath:OnChannelFinish( bInterrupted )
	if IsServer() then
