...
jungle_spirit_storm_cyclone = class({})

--------------------------------------------------------------------------------

function jungle_spirit_storm_cyclone:OnAbilityPhaseStart()
	if IsServer() then
		...
		ParticleManager:SetParticleControl( self.nPreviewFX, 15, Vector( 0, 150, 255 ) )
...
function jungle_spirit_storm_cyclone:OnAbilityPhaseInterrupted()
	if IsServer() then
...
function jungle_spirit_storm_cyclone:GetPlaybackRateOverride()
	return 0.4
end

--------------------------------------------------------------------------------

function jungle_spirit_storm_cyclone:OnSpellStart()
	if IsServer() then
		...
		for i = 1, self.projectile_count do 
			...
			info.vVelocity = -1 * ( RotatePosition( Vector( 0, 0, 0 ), angle, Vector( 1, 0, 0 ) ) ) * self.projectile_speed
...
function jungle_spirit_storm_cyclone:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		if hTarget ~= nil and ( not hTarget:IsMagicImmune() ) and ( not hTarget:IsInvulnerable() ) then
			...
			hTarget:AddNewModifier( self:GetCaster(), self, "modifier_jungle_spirit_storm_cyclone", { duration = self.debuff_duration } )
