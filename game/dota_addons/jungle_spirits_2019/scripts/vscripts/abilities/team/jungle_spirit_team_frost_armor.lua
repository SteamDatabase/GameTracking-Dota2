...
jungle_spirit_team_frost_armor = class({})

--------------------------------------------------------------------------------

function jungle_spirit_team_frost_armor:OnAbilityPhaseStart()
	if IsServer() then
...
function jungle_spirit_team_frost_armor:OnAbilityPhaseInterrupted()
	if IsServer() then
...
function jungle_spirit_team_frost_armor:GetPlaybackRateOverride()
	return 0.4
end

--------------------------------------------------------------------------------

function jungle_spirit_team_frost_armor:OnSpellStart()
	if IsServer() then
		...
		for _, hHero in ipairs( hAllHeroes ) do
			if hHero ~= nil and hHero:GetTeamNumber() == self:GetCaster():GetTeamNumber() then
				if hHero:IsRealHero() and hHero:IsOwnedByAnyPlayer() and hHero:IsClone() == false and hHero:IsTempestDouble() == false then
					hHero:AddNewModifier( self:GetCaster(), self, "modifier_lich_frost_armor", { duration = self:GetDuration() } )
				end
			end
		end
	end
