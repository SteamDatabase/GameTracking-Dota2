...
function HellbearThink()
	...
	if SmashAbility == nil then
		return 0.5
	end
	
	local nRange = SmashAbility:GetEffectiveCastRange( thisEntity:GetOrigin(), nil )
	--print( '^^^HELLBEAR CAST RANGE = ' .. nRange )

	local enemies = FindUnitsInRadius( thisEntity:GetTeamNumber(), thisEntity:GetOrigin(), nil, nRange, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO + DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_MAGIC_IMMUNE_ENEMIES + DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_CLOSEST, false )
	if #enemies == 0 then
		return 0.5
	end
