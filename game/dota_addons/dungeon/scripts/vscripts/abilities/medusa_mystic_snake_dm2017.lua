...
function medusa_mystic_snake_dm2017:OnSpellStart()
	...
	self.snake_max_mana_steal = self:GetSpecialValueFor( "snake_max_mana_steal" )
...
function medusa_mystic_snake_dm2017:OnProjectileHit( hTarget, vLocation )
	if IsServer() then
		...
		if ( hTarget == self:GetCaster() ) then
			if self.nTotalMana > self.snake_max_mana_steal then
				self.nTotalMana = self.snake_max_mana_steal
			end
