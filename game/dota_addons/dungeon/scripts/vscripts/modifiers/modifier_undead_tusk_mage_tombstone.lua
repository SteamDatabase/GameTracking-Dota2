...
function modifier_undead_tusk_mage_tombstone:OnIntervalThink()
	if IsServer() then
		...
		if #self.hSkeletons < nMaxSkels then
			--print( string.format( "modifier_undead_tusk_mage_tombstone - This tombstone owns less than %d skels, so making another one now", nMaxSkels ) )
