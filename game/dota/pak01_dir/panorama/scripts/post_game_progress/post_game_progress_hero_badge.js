...
function AnimateHeroBadgeXPIncreaseActionOld( panel, progress, xpAmount, xpValueStart, xpEarnedStart, xpLevelStart, resumeFromPreviousRow )
{
	this.panel = panel;
...
AnimateHeroBadgeXPIncreaseActionOld.prototype = new BaseAction();

AnimateHeroBadgeXPIncreaseActionOld.prototype.start = function ()
{
...
AnimateHeroBadgeXPIncreaseActionOld.prototype.update = function ()
{
	return this.seq.update();
}
AnimateHeroBadgeXPIncreaseActionOld.prototype.finish = function ()
{
	this.seq.finish();
}
...
function AnimateShardRewardActionOld( panel, label, shardAmount )
{
	this.panel = panel;
	this.label = label;
...
AnimateShardRewardActionOld.prototype = new BaseAction();

AnimateShardRewardActionOld.prototype.start = function ()
{
	var rowsContainer = this.panel.FindChildInLayoutFile( "HeroBadgeProgressRows" );
	var row = null;
...
AnimateShardRewardActionOld.prototype.update = function ()
{
	return this.seq.update();
}
AnimateShardRewardActionOld.prototype.finish = function ()
{
	this.seq.finish();
}
...
AnimateHeroBadgeLevelScreenAction.prototype.start = function ()
{
	...
	if ( this.data.hero_badge_progress_old )
	{
		...
		for ( var i = 0; i < this.data.hero_badge_progress_old.length; ++i )
		{
			...
			while ( xpRemaining > 0 )
			{
				...
				if ( xpToAnimate > 0 )
				{
					this.seq.actions.push( new SkippableAction( new AnimateHeroBadgeXPIncreaseActionOld( panel, this.data.hero_badge_progress_old[i], xpToAnimate, xpEarnedOnRow, xpEarned, xpLevel, xpEarnedOnRow != 0 ) ) );
		...
		if ( this.data.dota_plus_progress_old !== undefined )
		{
			if ( this.data.dota_plus_progress_old.tips !== undefined && this.data.dota_plus_progress_old.tips.length != 0 )
			{
				...
				this.seq.actions.push( new AnimateShardRewardActionOld( panel, '#DOTA_PlusPostGame_PlayerTips', nShardTips ) );
			}

			if ( this.data.dota_plus_progress_old.victory_prediction_shard_reward > 0 )
			{
				this.seq.actions.push( new AnimateShardRewardActionOld( panel, '#DOTA_PlusPostGame_VictoryPrediction', this.data.dota_plus_progress_old.victory_prediction_shard_reward ) );
			}

			if ( this.data.dota_plus_progress_old.cavern_crawl !== undefined )
			{
				this.seq.actions.push( new AnimateShardRewardActionOld( panel, '#DOTA_PlusPostGame_CavernCrawlProgress', this.data.dota_plus_progress_old.cavern_crawl.shard_amount ) );
			}

			if ( this.data.dota_plus_progress_old.role_call_shard_reward > 0 )
			{
			    this.seq.actions.push( new AnimateShardRewardActionOld( panel, '#DOTA_PlusPostGame_RoleCallProgress', this.data.dota_plus_progress_old.role_call_shard_reward ) );
			}
		}
	...
	if ( this.data.hero_relics_progress_old )
	{
		if ( this.data.hero_relics_progress_old.length > 0 )
		{
			...
			{
				stagger.actions.push( new AnimateHeroRelicProgressAction( this.data.hero_relics_progress_old[i], relicsList ) )
			}
