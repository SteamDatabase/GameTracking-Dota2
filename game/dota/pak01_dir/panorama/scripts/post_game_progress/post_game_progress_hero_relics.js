...
AnimateHeroRelicProgressAction.prototype.start = function ()
{
	...
	while ( nCurrentValue < this.data.ending_value )
	{
		...
		{
			this.seq.actions.push( new AnimateProgressBarAction( progressBar, nCurrentLevelValue, nValuePerLevel, 0.3 ) );
			this.seq.actions.push( new TriggerClassAction( levelUpLabel, 'ShowLevelUp' ) );
			this.seq.actions.push( new SetDialogVariableStringAction( this.panel, 'relic_level', ( nCurrentLevel + 1 ).toString() ) );
		...
		{
			this.seq.actions.push( new AnimateProgressBarAction( progressBar, nCurrentLevelValue, this.data.ending_value % nValuePerLevel, 0.3 ) );
