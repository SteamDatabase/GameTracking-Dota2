...
AddNewWeeklyProgressPanel = function(seq, parentPanel, i, data)
{
...
AnimateSpring2021WeeklyProgressSubpanelAction.prototype.start = function ()
{
	...
	for(var i = 0; i < this.data.length; ++i)
	{
		...
		AddNewWeeklyProgressPanel( this.seq, weeklyProgressPanelParent, i, data );
    }
...
AnimateSpring2021ScreenAction.prototype.start = function ()
{
    ...
    $.DispatchEvent( "DOTAWeeklyQuestPipBarSetLocalUserSeasonInfo", this.data.spring_2021_progress.battle_points_event_id, this.data.spring_2021_progress.active_season_id );
...
AnimateGenericArcanaScreenAction.prototype.start = function ()
{
	...
	var heroName = this.data.generic_arcana_progress.arcana_hero_name;
	var startScore = this.data.generic_arcana_progress.arcana_start_score;
	var endScore = this.data.generic_arcana_progress.arcana_end_score;
	...
	if ( typeof this.data.player_slot !== 'undefined' )
	{
		...
		unstyledHeroModel.SetScenePanelToPlayerHero( this.data.match_id, this.data.player_slot );
	}
	else
	{
		// Use this for testing when we don't actually have match data
		heroModel.SetScenePanelToLocalHero( this.data.hero_id );
		unstyledHeroModel.SetScenePanelToLocalHero( this.data.hero_id );
	}
	...
	var bundleItems = this.data.generic_arcana_progress.arcana_bundle_items;
	var bundleSlots = this.data.generic_arcana_progress.arcana_bundle_slots;
	var styleIndex = this.data.generic_arcana_progress.style_index;
	for (var i = 0; i < bundleItems.length; ++i)
	{
		heroModel.ReplaceEconItemSlot( bundleSlots[i], bundleItems[i], styleIndex );
	}
	
	var progress = panel.FindChildTraverse('GenericArcanaProgressMain');

	panel.SetDialogVariable('arcana_progress_max_score', this.data.generic_arcana_progress.arcana_max_score );
	...
	progressBar.min = 0;
	progressBar.max = this.data.generic_arcana_progress.arcana_max_score;
